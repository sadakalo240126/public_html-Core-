...
.diff-val.added { background: #dcfce7; color: #166534; font-weight: 600; }
.diff-val.changed { background: #fef9c3; color: #854d0e; font-weight: 600; }
.diff-val.removed { background: #fee2e2; color: #991b1b; font-weight: 600; }
...
<!-- =======================================
     MOBILE BOTTOM NAVIGATION BAR (প্লাস বাটন ছাড়া)
     ======================================= -->
<div class="mobile-nav">
  ...
  <!-- রিপোর্ট -->
  <a href="/Audit/" class="mobile-nav-item">
    <i class="bi bi-file-earmark-text-fill"></i>
    <span>রিপোর্ট</span>
  </a>

  <!-- ডাউনলোড -->
  <a href="/Backup/" class="mobile-nav-item">
    <i class="bi bi-backup"></i>
    <span>Backup</span>
  </a>

  <!-- কন্ট্রোল -->
  <a href="/admin/master_control.php" class="mobile-nav-item">
    <i class="bi bi-gear-fill"></i>
    <span>User</span>
  </a>

  <!-- ভার্সন -->
  <a href="/Settings/ServiceWorkerView.php" class="mobile-nav-item" onclick="showVersion()">
...
function showVersion() {
  const versionBtn = document.querySelector('.mobile-nav-item[onclick*="showVersion"]');
  if (versionBtn) {
    versionBtn.innerHTML = '<i class="bi bi-info-circle-fill"></i><span>লোডিং...</span>';
  }

  fetch('/sw_version.php', {
    cache: 'no-store',
    ...
    .then(function(data) {
      const version = data.app_version || data.version || '1.0.0';
      const buildDate = data.build_date || '';
      let message = 'অ্যাপ ভার্সন: ' + version;
      if (buildDate) message += '\nবিল্ড তারিখ: ' + buildDate;
      alert(message);
      if (versionBtn) {
        versionBtn.innerHTML = '<i class="bi bi-info-circle-fill"></i><span>ভার্সন</span>';
      }
    })
    .catch(function(error) {
      alert('ভার্সন: 1.0.0 (ডিফল্ট)\n\nতথ্য: ' + error.message);
      if (versionBtn) {
        versionBtn.innerHTML = '<i class="bi bi-info-circle-fill"></i><span>ভার্সন</span>';
      }
    });
...
function setActiveNavItem() {
  ...
  navItems.forEach(function(item) {
    ...
    if (href && currentPath.includes(href.replace('/index.php', '').replace('/Audit/', '').replace('/Control/', ''))) {
