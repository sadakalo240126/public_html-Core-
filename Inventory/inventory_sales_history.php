...
// ── Location Colors (DB → PHP badge) ─────────────────────
// বিক্রির সময় পণ্যটি কোথায় ছিল সেটার badge-এর জন্য
$locationColors = [
...
} catch (Throwable $e) { /* fallback colors */ }

function renderLocBadge(string $loc, array $colors): string {
    $c = $colors[$loc] ?? null;
    if (!$c) return '';
    return "<span style='display:inline-flex;align-items:center;gap:.2rem;padding:.12rem .4rem;"
         . "border-radius:.3rem;border:1.5px solid {$c['color']};background:{$c['bg']};"
         . "color:{$c['color']};font-size:.65rem;font-weight:800;white-space:nowrap;'>"
         . "<i class='fas {$c['icon']}' style='font-size:.55rem;'></i> {$c['label']}</span>";
}
...
if ($isAjax && $_POST['ajax_action'] === 'load_sales_history') {
    ...
    if (count($groupedByInvoice) > 0) {
        ...
        // Summary Card — আলাদা return হবে, sticky wrapper-এ বসবে
        $summaryHtml .= '<div class="sk-mobile-summary">';
        $summaryHtml .= '  <div class="sum-head"><i class="fas fa-calculator"></i> মোট হিসাব</div>';
        $summaryHtml .= '  <div class="sum-grid">';
        $summaryHtml .= '    <div class="sum-item"><span class="lbl">মোট পিস</span><span class="val">' . number_format($grandTotalPieces) . '</span></div>';
        $summaryHtml .= '    <div class="sum-item"><span class="lbl">মোট বিক্রি</span><span class="val">৳ ' . number_format($grandTotalAmount) . '</span></div>';
        if ($role === 'admin') {
            $summaryHtml .= '    <div class="sum-item" onclick="toggleAllProfits()" style="cursor:pointer;"><span class="lbl"><i class="fas fa-eye-slash" id="globalEyeIcon"></i> মুনাফা</span><span class="val"><span class="profit-mask">***</span><span class="profit-amt hidden">৳ ' . number_format($grandTotalProfit) . '</span></span></div>';
        }
        $summaryHtml .= '  </div>';
        $summaryHtml .= '</div>';
        ...
        foreach ($groupedByInvoice as $invNo => $invGroup) {
            ...
            foreach ($invGroup['items'] as $row) {
                ...
                // Location Badge — code-এর পাশে flex (overflow hidden-এ কাটবে না)
                $locBadge = renderLocBadge((string)($row['item_location'] ?? ''), $locationColors);
                $html .= '      <div class="s-code-row">';
                $html .= '        <span class="s-code">' . $pCode . '</span>';
                if ($locBadge !== '') {
                    $html .= '        <span class="s-loc-badge">' . $locBadge . '</span>';
                }
                $html .= '      </div>';
    ...
    echo json_encode(['html'=>$html, 'summaryHtml'=>$summaryHtml, 'totalPages'=>$totalPages, 'currentPage'=>$page]);
    exit;
}
?>
<!DOCTYPE html>
...
<head>
    ...
    <style>
        ...
        .s-code-row { display: flex; align-items: center; gap: .2rem; flex-wrap: nowrap; min-width: 0; }
        .s-code { font-weight: 800; font-size: 14px; line-height: 1.2; color: var(--sk-ink); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; flex-shrink: 1; min-width: 20px; }
        .s-loc-badge { flex-shrink: 0; }
        ...
        @media (max-width: 560px) {
            ...
            .s-img-code { min-width: 78px; max-width: 110px; gap: 6px; }
            .item-img-new { width: 36px; height: 36px; }
            .s-code-row { flex-direction: column; align-items: flex-start; gap: 1px; }
            .s-code { font-size: 12px; min-width: 0; flex-shrink: 0; }
            .s-loc-badge { transform: scale(.85); transform-origin: left center; }
            .s-name { display: none; }
        ...
        .sh-sticky .sk-section-title { margin-bottom: .3rem; padding-top: .1rem; }
        .sh-sticky .sk-section-title h2 { font-size: .85rem; margin: 0; }

        /* Summary Card compact (sticky-এর ভেতরে) */
        .sh-sticky .sk-mobile-summary {
            border-radius: .5rem;
            padding: .5rem .75rem;
            margin-bottom: .3rem;
        }
        .sh-sticky .sk-mobile-summary .sum-head { font-size: .6rem; margin-bottom: .25rem; }
        .sh-sticky .sk-mobile-summary .sum-grid { gap: .3rem; }
        .sh-sticky .sk-mobile-summary .sum-item { padding: .25rem .4rem; }
        .sh-sticky .sk-mobile-summary .sum-item .lbl { font-size: .5rem; }
        .sh-sticky .sk-mobile-summary .sum-item .val { font-size: .85rem; }

        /* Pagination card compact */
        #paginationCard {
        ...
        /* Location badge — ultra compact, code-এর পাশে */
        .loc-badge-sm {
            display: inline-flex; align-items: center; gap: .1rem;
            padding: .02rem .2rem; border-radius: .2rem;
            border: 1px solid; font-size: .55rem; font-weight: 800;
            white-space: nowrap; line-height: 1.4; vertical-align: middle;
            margin-left: .25rem;
        }
        .loc-badge-sm i { font-size: .48rem; }
    </style>
</head>
<body>
...
<main class="sk-container">
    ...
    <!-- ══ STICKY: Title + Summary + Pagination ══ -->
    <div class="sh-sticky">

        <div class="sk-section-title">
            <h2><i class="fas fa-receipt"></i> সেলস হিস্ট্রি</h2>
            <span class="sk-sub"><?= $role === 'admin' ? 'Admin View' : 'আজকের' ?></span>
        </div>

        <!-- Summary Card — JS দিয়ে populate হবে -->
        <div id="summaryArea"></div>

        <?php if($role === 'admin'): ?>
        <!-- Pagination + Filter — summary-এর নিচে -->
        <div class="sk-card" id="paginationCard">
            <div class="sk-row sk-row--between sk-row--wrap" style="gap:6px;">
                <div class="sk-row" id="paginationContainer" style="gap:4px; display:none;">
                    <button onclick="changePage(-1)" class="sk-pager__btn" id="prevBtn"><i class="fas fa-chevron-left"></i></button>
                    <span class="sk-pill sk-pill--ghost" style="font-size:.72rem;">Page <span id="pageNumDisplay">1</span> / <span id="totalPagesDisplay">1</span></span>
                    <button onclick="changePage(1)" class="sk-pager__btn" id="nextBtn"><i class="fas fa-chevron-right"></i></button>
                </div>
                <button type="button" onclick="toggleFilterDiv()" class="sk-btn sk-btn--accent sk-btn--sm" style="font-size:.72rem; padding:.3rem .7rem;"><i class="fas fa-filter"></i> ফিল্টার</button>
            </div>
            <div id="customFilterDiv" class="hidden" style="margin-top:10px; padding-top:10px; border-top:1px dashed var(--sk-line);">
                <div style="display:grid; grid-template-columns:1fr 1fr; gap:8px; margin-bottom:8px;">
                    <div class="sk-field" style="margin-bottom:0;"><input type="date" id="startDate" class="sk-input" style="font-size:.75rem;" value="<?= date('Y-m-d') ?>"></div>
                    <div class="sk-field" style="margin-bottom:0;"><input type="date" id="endDate" class="sk-input" style="font-size:.75rem;" value="<?= date('Y-m-d') ?>"></div>
                </div>
                <div class="sk-row" style="gap:6px;">
                    <button type="button" onclick="setMode('custom')" class="sk-btn sk-btn--ink sk-grow" style="font-size:.75rem;"><i class="fas fa-search"></i> খুঁজুন</button>
                    <button type="button" onclick="setMode('recent')" class="sk-btn sk-btn--ghost sk-grow" style="font-size:.75rem;"><i class="fas fa-undo"></i> আজকে রিসেট</button>
                </div>
            </div>
        </div>
        <?php endif; ?>
    ...
    <div id="historyContainer" style="min-height:200px; margin-top:.5rem;">
...
function loadHistoryData() {
    $('#historyContainer').html('<div class="sk-empty"><i class="fas fa-spinner fa-spin"></i><p>লোড হচ্ছে...</p></div>');
    ...
    $.ajax({
        ...
        success: function(res) {
            ...
            // Summary — sticky wrapper-এ, list — নিচে
            $('#summaryArea').html(res.summaryHtml || '');
