...
// StorageLocationHelper — টাইমলাইন লগের জন্য
$slhPath = __DIR__ . '/Helpers/StorageLocationHelper.php';
if (file_exists($slhPath)) require_once $slhPath;
    ...
    if ($action === 'process_return') {
        try {
            ...
            if ($status === 'approved') {
                ...
                // ── Location Timeline log: 'returned' ──
                if (class_exists('StorageLocationHelper')) {
                    StorageLocationHelper::logEvent($conn, [
                        'product_code' => $prodCode,
                        'event_type'   => 'returned',
                        'pieces'       => $returnPieces,
                        'invoice_no'   => $invoiceNo,
                        'note'         => 'কাস্টমার রিটার্ন' . ($note !== '' ? ' — ' . $note : ''),
                        'done_by'      => $uid,
